...
namespace Datiss.Budget.Reports.Excel
{
    ...
    public static class CostForcastTransferWsExcel
    {
        private const string _sheetName = "CostForcastTransferWs";

        public static XLWorkbook ExportExcel(this IEnumerable<CostForcastTransferWsDTO> items)
        {
            ...
            sheet.Cell(1, 7).Value = "روش کندمان";
            sheet.Cell(1, 8).Value = "جنس لوله";
            sheet.Cell(1, 9).Value = "قطر لوله";
            sheet.Cell(1, 10).Value = "طول اجرا";
            sheet.Cell(1, 11).Value = "هزینه هر متر خرید لوله";
            sheet.Cell(1, 12).Value = "هزینه هر متر اجرا";
            sheet.Cell(1, 13).Value = "جمع کل هزینه ها (هزار ریال)";
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 7).Value = item.MethodTypeDisplay;
                sheet.Cell(row, 8).Value = item.TubeTypeDisplay;
                sheet.Cell(row, 9).Value = item.DiameterPipeTypeDisplay;
                sheet.Cell(row, 10).Value = item.Lenth;
                ...
                sheet.Cell(row, 11).Value = item.PipeCost;
                ...
                sheet.Cell(row, 12).Value = item.RunCost;
                ...
                sheet.Cell(row, 13).Value = item.TotalCost;
                ...
                sheet.Cell(row, 14).Value = item.ExtensionTypeDisplay;
                sheet.Cell(row, 15).Value = item.SuggestedBudgetTopicTypeDisplay;
        ...
        public static XLWorkbook GetImportTemplate(this CostForcastTransferWsImportViewModel model, int year)
        {
            if (!model.Items.Any())
                return null;
            ...
            sheet.Cell(1, 7).Value = "روش کندمان";
            sheet.Cell(1, 8).Value = "کد روش کندمان";
            sheet.Cell(1, 9).Value = "جنس لوله";
            sheet.Cell(1, 10).Value = "کد جنس لوله";
            sheet.Cell(1, 11).Value = "قطر لوله";
            sheet.Cell(1, 12).Value = "کد قطر لوله";
            sheet.Cell(1, 13).Value = "این پروژه در سال بودجه به بهره برداری رسیده؟";
            sheet.Cell(1, 14).Value = "کد بهره برداری";
            sheet.Cell(1, 15).Value = "سر فصل کلی در بودجه ";
            sheet.Cell(1, 16).Value = "کد سر فصل کلی در بودجه ";
            ...
            row = 2;
            foreach (var item in model.MethodTypeSource)
            {
            ...
            sheet.Cell(20, 6).Value = "کد نوع کندمان";
            sheet.Cell(20, 7).Value = "کد روش کندمان";
            sheet.Cell(20, 8).Value = "کد جنس لوله";
            sheet.Cell(20, 9).Value = "کد قطر لوله";
            sheet.Cell(20, 10).Value = "طول اجرا";
            sheet.Cell(20, 11).Value = "هزینه هر متر خرید لوله";
            sheet.Cell(20, 12).Value = "هزینه هر متر اجرا";
            sheet.Cell(20, 13).Value = "جمع کل هزینه ها (هزار ریال)";
            sheet.Cell(20, 14).Value = "کد بهره برداری";
            sheet.Cell(20, 15).Value = "کد سر فصل کلی در بودجه";
            ...
            var range = sheet.Range(20, 1, row - 1, 15);
