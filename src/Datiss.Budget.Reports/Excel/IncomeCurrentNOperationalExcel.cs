...
{
    public static class IncomeCurrentNOperationalExcel
    {
        private const string _sheetName = "IncomeCurrentNOperational";

        public static XLWorkbook ExportExcel(this IEnumerable<IncomeCurrentNOperationalDTO> items)
        {
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.NOICTypeDisplay;
                sheet.Cell(row, 3).DataType = XLDataType.Text;
                sheet.Cell(row, 4).Value = item.NOICPrice;
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<IncomeCurrentNOperationalDTO> items, int year)
        {
            if (items == null || !items.Any())
                return null;
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.NOICTypeDisplay;
                sheet.Cell(row, 4).Value = item.NOICTypeId;
