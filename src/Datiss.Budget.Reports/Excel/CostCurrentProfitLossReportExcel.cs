...
{
    public static class CostCurrentProfitLossReportExcel
    {
        private const string _sheetName = "CostCurrentProfitLossReport";

        public static XLWorkbook ExportExcel(this IEnumerable<CostCurrentProfitLossReportDTO> items)
        {
            ...
            sheet.Cell(1, 6).Value = "عملکرد سال  ما قبل";
            sheet.Cell(1, 7).Value = "مصوب سال ما قبل";
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<CostCurrentProfitLossReportDTO> items, int year)
        {
            if (items == null || !items.Any())
                return null;
