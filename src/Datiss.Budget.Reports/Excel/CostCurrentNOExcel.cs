...
namespace Datiss.Budget.Reports.Excel
{
    ...
    public static class CostCurrentNOExcel
    {
        private const string _sheetName = "CostCurrentNO";

        public static XLWorkbook ExportExcel(this IEnumerable<CostCurrentNODTO> items)
        {
            ...
            sheet.Cell(1, 4).Value = "پیش بینی مبلغ";
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.CostCurrentNoTypeDisplay;
                sheet.Cell(row, 3).DataType = XLDataType.Text;
                sheet.Cell(row, 4).Value = item.ForcastFee;
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<CostCurrentNODTO> items, int year)
        {
            if (items == null || !items.Any())
                return null;
            ...
            sheet.Cell(2, 5).Value = "پیش بینی مبلغ";
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.CostCurrentNoTypeDisplay;
                sheet.Cell(row, 4).Value = item.CostCurrentNoTypeId;
