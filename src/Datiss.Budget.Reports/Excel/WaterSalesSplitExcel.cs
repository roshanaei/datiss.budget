...
{
    public static class WaterSalesSplitExcel
    {
        private const string _sheetName = "WaterSalesSplit";

        public static XLWorkbook ExportExcel(this IEnumerable<WaterSalesSplitDTO> items)
        {
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 4).Value = item.WPipeDiameterDisplay;
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<WaterSalesSplitDTO> items, int year)
        {
            if (items == null || !items.Any())
                return null;
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 5).Value = item.WPipeDiameterDisplay;
                sheet.Cell(row, 6).Value = item.WPipeDiameterId;
