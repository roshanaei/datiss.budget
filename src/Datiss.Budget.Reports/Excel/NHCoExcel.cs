    ...
    public static class NHCoExcel
    {
        ...
        public static XLWorkbook ExportExcel(this IEnumerable<NHCoDTO> items)
        {
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.ActivityType;
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<NHCoDTO> items, int year ,string activity)
        {
            if (items == null || !items.Any())
                return null;
            ...
            sheet.Cell(2, 1).Value = "ورود اطلاعات برای مشارکت : " + activity;
            sheet.Range(2, 1, 1, 5).Merge();

            sheet.Cell(3, 1).Value = "عنوان سازمان";
            sheet.Cell(3, 2).Value = "کد سازمان";
            sheet.Cell(3, 3).Value = "حد ظرفیت برای متغیر اول";
            sheet.Cell(3, 4).Value = "مبلغ ثابت هزینه مشارکت";
            sheet.Cell(3, 5).Value = "مبلغ هزینه مشارکت متغیر اول";
            sheet.Cell(3, 6).Value = "مبلغ هزینه مشارکت متغیر دوم";
