...
{
    public static class PerformanceEvaluationExcel
    {
        private const string _sheetName = "PerformanceEvaluation";

        public static XLWorkbook ExportExcel(this IEnumerable<PerformanceEvaluationDTO> items)
        {
            ...
            sheet.Cell(1, 4).Value = "هدف سالیانه";
            sheet.Cell(1, 5).Value = "بودجه";
            sheet.Cell(1, 6).Value = "عملکرد";
            sheet.Cell(1, 7).Value = "درصد تحقق";
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.TableFieldDisplay;
                sheet.Cell(row, 4).Value = item.Target;
                ...
                sheet.Cell(row, 5).Value = item.Budget;
                ...
                sheet.Cell(row, 6).Value = item.Operation;
                ...
                sheet.Cell(row, 7).Value = item.PercentRealization;
        ...
        public static XLWorkbook GetImportTemplate(this IEnumerable<PerformanceEvaluationDTO> items, int year)
        {
            if (items == null || !items.Any())
                return null;
            ...
            sheet.Cell(1, 3).Value = "ارزیابی برای چند ماه است ؟(در سلول بعدی وارد کنید)";
            sheet.Cell("E1").Style.Alignment.Horizontal = XLAlignmentHorizontalValues.Center;

            sheet.Range(1, 1, 1, 2).Merge();
            sheet.Range(1, 3, 1, 4).Merge();

            sheet.Cell(2, 1).Value = "عنوان سازمان";
            sheet.Cell(2, 2).Value = "کد سازمان";
            sheet.Cell(2, 3).Value = "عنوان";
            sheet.Cell(2, 4).Value = "کد عنوان";
            sheet.Cell(2, 5).Value = "عملکرد";
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                sheet.Cell(row, 3).Value = item.TableFieldDisplay;
                sheet.Cell(row, 4).Value = item.TableFieldId;
