...
{
    public class DirectoratReportService : IDirectoratReportService
    {
        ...
        private readonly DbSet<DirectoratReport> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public DirectoratReportService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<DirectoratReport>();
        ...
        private IQueryable<DirectoratReport> Query()
            => _dbSet.AsNoTracking();

        public async Task<DirectoratReport> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<DirectoratReportDTO>> UpdateAsync(UpdateDirectoratReportDTO model)
        {
            ...
            try
            {
                if (await checkLogicAsync(model.YearId, model.OrganizationId, model.SectionTypeId, model.Id))
                {
                    ...
                    {
                        return ValidationResult<DirectoratReportDTO>.Failed(
                            string.Format(ServiceMessages.ImportExcelCalculationField)
                            );
                    }

                    var result = entity.Adapt<DirectoratReportDTO>();
                    ...
                    return ValidationResult<DirectoratReportDTO>.Success(result);
                }
            }
            catch (DisbaledYearDataInputException)
            {
                return ValidationResult<DirectoratReportDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<DirectoratReport> childrens = new DirectoratReport[] { };
        ...
        public async Task<PagedResult<DirectoratReportDTO>> GetListAsync(DirectoratReportFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<DirectoratReportDTO>
            {
            ...
            result.Items = await query.Include(x => x.FinanceYear)
                                        ...
                                        .Select(x => x.Adapt<DirectoratReportDTO>())
                                        .ToListAsync();
        ...
        public async Task<ValidationResult> CalculationAsync(int yearId, int organizationId)
        {
            ...
            await _uow.ExecuteScalar<ValidationResult>(
                                    "[dbo].[DirectoratReport_Insert] @YearId, @OrganizationId",
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<DirectoratReport>();
            ...
            if (selfData.Any())
            {
                foreach (var item in selfData)
                {
                    ...
                    var entity = item.Adapt<DirectoratReport>();
        ...
        public async Task<IEnumerable<DirectoratReportDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
            var filter = new DirectoratReportFilterDTO
            {
        ...
        {
            var data = await _excelService.ImportAsync<DirectoratReportImportModel>
               (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<DirectoratReport>>();
            ...
            var sectionTypes = _constSet.Where(x => x.Parent.ConstantKey == ConstantKeys.__DirectorateReportType &&
        ...
        private async Task<IQueryable<DirectoratReport>> setFilter(
            IQueryable<DirectoratReport> query,
            DirectoratReportFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<DirectoratReport>();
        ...
        private IQueryable<DirectoratReport> setOrder(
            IQueryable<DirectoratReport> query,
            string orderBy = "id",
            bool desc = false)
        {
        ...
        private async Task<IEnumerable<DirectoratReport>> getChildren(
        ...
        private async Task<IEnumerable<DirectoratReport>> getChildrenData(
            int parentOrganizationId,
            int yearId,
            int targetYearId)
        {
        ...
        private async Task<DirectoratReport> checkLogicAsync(
             int yearId,
             int organizationId,
             int sectionTypeId)
        {
