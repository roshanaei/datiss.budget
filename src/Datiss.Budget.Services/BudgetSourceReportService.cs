...
{
    public class BudgetSourceReportService : IBudgetSourceReportService
    {
        ...
        private readonly DbSet<BudgetSourceReport> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public BudgetSourceReportService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<BudgetSourceReport>();
        ...
        private IQueryable<BudgetSourceReport> Query()
            => _dbSet.AsNoTracking();

        public async Task<BudgetSourceReport> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<BudgetSourceReportDTO>> UpdateAsync(UpdateBudgetSourceReportDTO model)
        {
            ...
            try
            {
                if (await checkLogicAsync(model.YearId, model.OrganizationId, model.SectionTypeId, model.Id))
                {
                    ...
                    {
                        return ValidationResult<BudgetSourceReportDTO>.Failed(
                            string.Format(ServiceMessages.ImportExcelCalculationField)
                            );
                    }

                    var result = entity.Adapt<BudgetSourceReportDTO>();

                    result.OrganizationDisplay = organizationDisplay;
                    result.SectionTypeDisplay = model.SectionTypeTitle;
                    result.Year = (await _yearSet.FindAsync(model.YearId)).Year;
                                  
                    return ValidationResult<BudgetSourceReportDTO>.Success(result);
                }
            }
            catch (DisbaledYearDataInputException)
            {
                return ValidationResult<BudgetSourceReportDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<BudgetSourceReport> childrens = new BudgetSourceReport[] { };
        ...
        public async Task<PagedResult<BudgetSourceReportDTO>> GetListAsync(BudgetSourceReportFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<BudgetSourceReportDTO>
            {
            ...
            result.Items = await query.Include(x => x.FinanceYear)
                                        ...
                                        .Select(x => x.Adapt<BudgetSourceReportDTO>())
                                        .ToListAsync();
        ...
        public async Task<ValidationResult> CalculationAsync(int yearId, int organizationId)
        {
            ...
            await _uow.ExecuteScalar<ValidationResult>(
                                    "[dbo].[BudgetSourceReport_Calculation] @YearId, @OrganizationId",
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<BudgetSourceReport>();
            ...
            if (selfData.Any())
            {
                foreach (var item in selfData)
                {
                    ...
                    var entity = item.Adapt<BudgetSourceReport>();
        ...
        public async Task<IEnumerable<BudgetSourceReportDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
            var filter = new BudgetSourceReportFilterDTO
            {
        ...
        {
            var data = await _excelService.ImportAsync<BudgetSourceReportImportModel>
               (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<BudgetSourceReport>>();
        ...
        private async Task<IQueryable<BudgetSourceReport>> setFilter(
            IQueryable<BudgetSourceReport> query,
            BudgetSourceReportFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<BudgetSourceReport>();
        ...
        private IQueryable<BudgetSourceReport> setOrder(
            IQueryable<BudgetSourceReport> query,
            string orderBy = "id",
            bool desc = false)
        {
        ...
        private async Task<IEnumerable<BudgetSourceReport>> getChildren(
        ...
        private async Task<IEnumerable<BudgetSourceReport>> getChildrenData(
            int parentOrganizationId,
            int yearId,
            int targetYearId)
        {
        ...
        private async Task<BudgetSourceReport> checkLogicAsync(
             int yearId,
             int organizationId,
             int sectionTypeId)
        {
