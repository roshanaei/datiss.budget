...
namespace Datiss.Budget.Services
{
    ...
    public class CostForcastTransferWsService : ICostForcastTransferWsService
    {
        ...
        private readonly DbSet<CostForcastTransferWs> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public CostForcastTransferWsService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<CostForcastTransferWs>();
        ...
        private IQueryable<CostForcastTransferWs> Query()
            => _dbSet.AsNoTracking();

        public async Task<CostForcastTransferWs> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<CostForcastTransferWsDTO>> CreateAsync(CreateCostForcastTransferWsDTO model)
        {
            model.CheckArgumentIsNull(nameof(model));

            var entity = model.Adapt<CostForcastTransferWs>();
            ...
            try
            {
                ...
                {
                    return ValidationResult<CostForcastTransferWsDTO>.Failed(
                ...
                return ValidationResult<CostForcastTransferWsDTO>.Success(result);
            }
            catch (DisbaledYearDataInputException)
            {
                return ValidationResult<CostForcastTransferWsDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }


        }

        public async Task<ValidationResult<CostForcastTransferWsDTO>> UpdateAsync(UpdateCostForcastTransferWsDTO model)
        {
            model.CheckArgumentIsNull(nameof(model));
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<CostForcastTransferWs> childrens = new CostForcastTransferWs[] { };
        ...
        public async Task<IEnumerable<CalculationItemData>> CalculationAsync(int yearId, int organizationId)
        {
            ...
            {
                Key = "CostForcastTransferWs_Cal1",
                Value = await _uow.ExecuteScalar<long>(
                                    "[dbo].[CostForcastTransferWs_Cal1] @YearId, @OrganizationId",
                                    parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal2",
                Value = await _uow.ExecuteScalar<long>(
                                    "[dbo].[CostForcastTransferWs_Cal2] @YearId, @OrganizationId",
                                    parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal3",
                Value = await _uow.ExecuteScalar<long>(
                         "[dbo].[CostForcastTransferWs_Cal3] @YearId, @OrganizationId",
                         parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal4",
                Value = await _uow.ExecuteScalar<long>(
                         "[dbo].[CostForcastTransferWs_Cal4] @YearId, @OrganizationId",
                         parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal5",
                Value = await _uow.ExecuteScalar<long>(
                         "[dbo].[CostForcastTransferWs_Cal5] @YearId, @OrganizationId",
                         parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal6",
                Value = await _uow.ExecuteScalar<long>(
             "[dbo].[CostForcastTransferWs_Cal6] @YearId, @OrganizationId",
             parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal7",
                Value = await _uow.ExecuteScalar<long>(
             "[dbo].[CostForcastTransferWs_Cal7] @YearId, @OrganizationId",
             parameters: sqlParams.ToArray())
            });

            result.Add(new CalculationItemData
            {
                Key = "CostForcastTransferWs_Cal8",
                Value = await _uow.ExecuteScalar<long>(
             "[dbo].[CostForcastTransferWs_Cal8] @YearId, @OrganizationId",
             parameters: sqlParams.ToArray())
            });
        ...
        public async Task<PagedResult<CostForcastTransferWsDTO>> GetListAsync(CostForcastTransferWsFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<CostForcastTransferWsDTO>
            {
            ...
            result.Items = await query.Include(x => x.FinanceYear)
                                    ...
                                    .Select(x => x.Adapt<CostForcastTransferWsDTO>())
                                    .ToListAsync();
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<CostForcastTransferWs>();
            ...
            if (selfData.Any())
            {
                foreach (var item in selfData)
                {
                    ...
                    item.Id = 0;
                    var entity = item.Adapt<CostForcastTransferWs>();
        ...
        {
            var data = await _excelService.ImportAsync<CostForcastTransferWsImportModel>
        ...
        public async Task<IEnumerable<CostForcastTransferWsDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
        ...
        private async Task<IQueryable<CostForcastTransferWs>> setFilter(
            IQueryable<CostForcastTransferWs> query,
            CostForcastTransferWsFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<CostForcastTransferWs>();
        ...
        private IQueryable<CostForcastTransferWs> setOrder(
           IQueryable<CostForcastTransferWs> query,
           string orderBy = "id",
           bool desc = false)
        {
        ...
        private async Task<IEnumerable<CostForcastTransferWs>> getChildrenData(
        ...
        private async Task<IEnumerable<CostForcastTransferWs>> getChildren(
            int parentOrganizationId,
            int yearId)
        {
            var children = await _orgDbSet
