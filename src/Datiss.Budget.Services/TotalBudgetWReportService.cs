...
{
    public class TotalBudgetWReportService : ITotalBudgetWReportService
    {
        ...
        private readonly DbSet<TotalBudgetWReport> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public TotalBudgetWReportService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<TotalBudgetWReport>();
        ...
        private IQueryable<TotalBudgetWReport> Query()
            => _dbSet.AsNoTracking();

        public async Task<TotalBudgetWReport> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<TotalBudgetWReportDTO>> UpdateAsync(UpdateTotalBudgetWReportDTO model)
        {
            ...
            try
            {
                if (await checkLogicAsync(model.YearId, model.OrganizationId, model.SectionTypeId, model.Id))
                {
                    ...
                    {
                        return ValidationResult<TotalBudgetWReportDTO>.Failed(
                            string.Format(ServiceMessages.ImportExcelCalculationField)
                            );
                    }

                    var result = entity.Adapt<TotalBudgetWReportDTO>();
            ...
            {
                return ValidationResult<TotalBudgetWReportDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<TotalBudgetWReport> childrens = new TotalBudgetWReport[] { };
        ...
        public async Task<PagedResult<TotalBudgetWReportDTO>> GetListAsync(TotalBudgetWReportFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<TotalBudgetWReportDTO>
            {
        ...
        public async Task<ValidationResult> CalculationAsync(int yearId, int organizationId)
        {
            ...
            await _uow.ExecuteScalar<ValidationResult>(
                                    "[dbo].[TotalBudgetWReport_Calculation] @YearId, @OrganizationId",
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<TotalBudgetWReport>();
            ...
            if (selfData.Any())
            {
                foreach (var item in selfData)
                {
                    ...
                    var entity = item.Adapt<TotalBudgetWReport>();
        ...
        public async Task<IEnumerable<TotalBudgetWReportDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
        ...
        {
            var data = await _excelService.ImportAsync<TotalBudgetWReportImportModel>
               (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<TotalBudgetWReport>>();
        ...
        private async Task<IQueryable<TotalBudgetWReport>> setFilter(
            IQueryable<TotalBudgetWReport> query,
            TotalBudgetWReportFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<TotalBudgetWReport>();
        ...
        private IQueryable<TotalBudgetWReport> setOrder(
            IQueryable<TotalBudgetWReport> query,
            string orderBy = "id",
            bool desc = false)
        {
        ...
        private async Task<IEnumerable<TotalBudgetWReport>> getChildren(
        ...
        private async Task<IEnumerable<TotalBudgetWReport>> getChildrenData(
            int parentOrganizationId,
            int yearId,
            int targetYearId)
        {
        ...
        private async Task<TotalBudgetWReport> checkLogicAsync(
             int yearId,
             int organizationId,
             int sectionTypeId)
        {
