...
namespace Datiss.Budget.Services
{
    ...
    public class CostCurrentWaterSourcePriceService : ICostCurrentWaterSourcePriceService
    {
        ...
        private readonly DbSet<CostCurrentWaterSourcePrice> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public CostCurrentWaterSourcePriceService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<CostCurrentWaterSourcePrice>();
        ...
        private IQueryable<CostCurrentWaterSourcePrice> Query()
            => _dbSet.AsNoTracking();

        public async Task<CostCurrentWaterSourcePrice> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<CostCurrentWaterSourcePriceDTO>> CreateAsync(CreateCostCurrentWaterSourcePriceDTO model)
        {
            model.CheckArgumentIsNull(nameof(model));

            var entity = new CostCurrentWaterSourcePrice
            {
                YearId = model.YearId,
            ...
            try
            {
                if (await checkLogicAsync(model.YearId, model.OrganizationId, model.WaterSourceTypeId))
                {
                    ...
                    var result = entity.Adapt<CostCurrentWaterSourcePriceDTO>();

                    result.WaterSourceTypeDisplay = model.WaterSourceTypeTitle;
                    result.OrganizationDisplay = organizationDisplay;
                    result.Year = (await _yearSet.FindAsync(model.YearId)).Year;

                    return ValidationResult<CostCurrentWaterSourcePriceDTO>.Success(result);
                }
            }
            catch (DisbaledYearDataInputException)
            {
                return ValidationResult<CostCurrentWaterSourcePriceDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }
        ...
        public async Task<ValidationResult<CostCurrentWaterSourcePriceDTO>> UpdateAsync(UpdateCostCurrentWaterSourcePriceDTO model)
        {
            model.CheckArgumentIsNull(nameof(model));
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<CostCurrentWaterSourcePrice> childrens = new CostCurrentWaterSourcePrice[] { };
        ...
        public async Task<IEnumerable<CalculationItemData>> CalculationAsync(int yearId, int organizationId)
        {
            ...
            {
                Key = "CostCurrentWaterSourcePrices_Cal1",
                Value = await _uow.ExecuteScalar<int>(
                        "[dbo].[CostCurrentWaterSourcePrices_Cal1] @YearId, @OrganizationId",
        ...
        public async Task<PagedResult<CostCurrentWaterSourcePriceDTO>> GetListAsync(DefaultFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<CostCurrentWaterSourcePriceDTO>
            {
            ...
            result.Items = await query.Include(x => x.FinanceYear)
                                    ...
                                    .Select(x => new CostCurrentWaterSourcePriceDTO
                                    {
                                        Id = x.Id,
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<CostCurrentWaterSourcePrice>();
        ...
        {
            var data = await _excelService.ImportAsync<CostCurrentWaterSourcePriceImportModel>
                (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<CostCurrentWaterSourcePrice>>();
            ...
            foreach (var org in existOrgs)
            {
                ...
                foreach (var item in waterSourcetypes)
                {
                    var existDWTypeInExcel = records.Any(_ => _.WaterSourceTypeId == item.Id &&
        ...
        public async Task<IEnumerable<CostCurrentWaterSourcePriceDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
        ...
        public async Task<Stream> ExportExcelAsync(DefaultFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));
        ...
        private async Task<IQueryable<CostCurrentWaterSourcePrice>> setFilter(
            IQueryable<CostCurrentWaterSourcePrice> query,
            DefaultFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<CostCurrentWaterSourcePrice>();
            ...
            if (filter.Search.IsNotNullOrEmpty())
            {
                ...
                query = query.Where(_ => _.Organization.Title.ToUpper().Contains(filter.Search) ||
                                         _.WaterSourceType.Title.ToUpper().Contains(filter.Search));
            }

            return query;
        }

        private IQueryable<CostCurrentWaterSourcePrice> setOrder(
           IQueryable<CostCurrentWaterSourcePrice> query,
           string orderBy = "id",
           bool desc = false)
        {
        ...
        private async Task<IEnumerable<CostCurrentWaterSourcePrice>> getChildrenData(
        ...
        private async Task<IEnumerable<CostCurrentWaterSourcePrice>> getChildren(
            int parentOrganizationId,
            int yearId)
        {
            var children = await _orgDbSet
