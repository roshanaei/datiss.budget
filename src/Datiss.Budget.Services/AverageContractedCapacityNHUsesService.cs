...
namespace Datiss.Budget.Services
{
    public class AverageContractedCapacityNHUsesService : IAverageContractedCapacityNHUsesService
    {
        ...
        public async Task<IEnumerable<CalculationItemData>> CalculationAsync(int yearId, int organizationId)
        {
            ...
            {
                Key = "AverageContractedCapacityNHUses_Cal1",
                Value = await _uow.ExecuteScalar<int>(
                        "[dbo].[AverageContractedCapacityNHUses_Cal1] @YearId, @OrganizationId",
        ...
        public async Task<PagedResult<AverageContractedCapacityNHUsesDTO>> GetListAsync(AverageContractedCapacityNHUsesFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<AverageContractedCapacityNHUsesDTO>
            {
        ...
        {
            var data = await _excelService.ImportAsync<AverageContractedCapacityNHUsesImportModel>
                (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<AverageContractedCapacityNHUses>>();
