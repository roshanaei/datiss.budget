...
{
    public class TotalBudgetWsReportService : ITotalBudgetWsReportService
    {
        ...
        private readonly DbSet<TotalBudgetWsReport> _dbSet;
        private readonly DbSet<Organization> _orgDbSet;
        private readonly DbSet<FinanceYear> _yearSet;
        private readonly DbSet<Constant> _constSet;

        public TotalBudgetWsReportService(
            ...
            IOrganizationService organizationService)
        {
            ...
            _dbSet = _uow.Set<TotalBudgetWsReport>();
        ...
        private IQueryable<TotalBudgetWsReport> Query()
            => _dbSet.AsNoTracking();

        public async Task<TotalBudgetWsReport> GetByIdAsync(int id)
        {
            var entity = await _dbSet.FindAsync(id);
            return await Task.FromResult(entity);
        }

        public async Task<ValidationResult<TotalBudgetWsReportDTO>> UpdateAsync(UpdateTotalBudgetWsReportDTO model)
        {
            ...
            try
            {
                if (await checkLogicAsync(model.YearId, model.OrganizationId, model.SectionTypeId, model.Id))
                {
                    ...
                    {
                        return ValidationResult<TotalBudgetWsReportDTO>.Failed(
                            string.Format(ServiceMessages.ImportExcelCalculationField)
                            );
                    }

                    var result = entity.Adapt<TotalBudgetWsReportDTO>();
                    ...
                    return ValidationResult<TotalBudgetWsReportDTO>.Success(result);
                }
            }
            catch (DisbaledYearDataInputException)
            {
                return ValidationResult<TotalBudgetWsReportDTO>.Failed(ServiceMessages.Logic_InputDisableYearData);
            }
        ...
        public async Task<OrganizationDeleteDataResult> HardDeleteAsync(int yearId, int organizationId)
        {
            ...
            IEnumerable<TotalBudgetWsReport> childrens = new TotalBudgetWsReport[] { };
        ...
        public async Task<PagedResult<TotalBudgetWsReportDTO>> GetListAsync(TotalBudgetWsReportFilterDTO filter)
        {
            filter.CheckArgumentIsNull(nameof(filter));

            var result = new PagedResult<TotalBudgetWsReportDTO>
            {
            ...
            result.Items = await query.Include(x => x.FinanceYear)
                                        ...
                                        .Select(x => x.Adapt<TotalBudgetWsReportDTO>())
                                        .ToListAsync();
        ...
        public async Task CopyAsync(int sourceYearId, int sourceOrgId, int destYearId)
        {
            ...
            var result = new List<TotalBudgetWsReport>();
            ...
            if (selfData.Any())
            {
                foreach (var item in selfData)
                {
                    ...
                    var entity = item.Adapt<TotalBudgetWsReport>();
        ...
        public async Task<IEnumerable<TotalBudgetWsReportDTO>> GetExportItemsAsync(int yearId, int organizationId)
        {
        ...
        {
            var data = await _excelService.ImportAsync<TotalBudgetWsReportImportModel>
               (fileInfo, sheetIndex: 0, minRowNum: 2);

            var records = data.Adapt<List<TotalBudgetWsReport>>();
            ...
            var sectionTypes = _constSet.Where(x => x.Parent.ConstantKey == ConstantKeys.__GeneralWasteReportType &&
        ...
        private async Task<IQueryable<TotalBudgetWsReport>> setFilter(
            IQueryable<TotalBudgetWsReport> query,
            TotalBudgetWsReportFilterDTO filter)
        {
            query.CheckArgumentIsNull(nameof(query));
            filter.CheckArgumentIsNull(nameof(filter));

            var predicate = PredicateBuilder.New<TotalBudgetWsReport>();
        ...
        private IQueryable<TotalBudgetWsReport> setOrder(
            IQueryable<TotalBudgetWsReport> query,
            string orderBy = "id",
            bool desc = false)
        {
        ...
        private async Task<IEnumerable<TotalBudgetWsReport>> getChildren(
        ...
        private async Task<IEnumerable<TotalBudgetWsReport>> getChildrenData(
            int parentOrganizationId,
            int yearId,
            int targetYearId)
        {
        ...
        private async Task<TotalBudgetWsReport> checkLogicAsync(
             int yearId,
             int organizationId,
             int sectionTypeId)
        {
