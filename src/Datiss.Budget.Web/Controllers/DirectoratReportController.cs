...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class DirectoratReportController : Controller
    {
        public const string Name = "DirectoratReport";
        ...
        private readonly ILogger<DirectoratReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly IDirectoratReportService _directoratReportService;
        ...
        public DirectoratReportController(
            ILogger<DirectoratReportController> logger,
            IWebHostEnvironment environment,
            IDirectoratReportService directoratReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _directoratReportService = directoratReportService ?? throw new ArgumentNullException(nameof(directoratReportService));
        ...
        public async Task<IActionResult> Edit(UpdateDirectoratReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateDirectoratReportDTO>();
            var result = await _directoratReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<DirectoratReportViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            var myfilter = TempData.Get<DirectoratReportFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<DirectoratReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _directoratReportService.GetListAsync(filter);
            var model = result.Adapt<DirectoratReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(DirectoratReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<DirectoratReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _directoratReportService.GetListAsync(filter);
            model = result.Adapt<DirectoratReportIndexViewModel>();
            model.Filter = filter.Adapt<DirectoratReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _directoratReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _directoratReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _directoratReportService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            foreach (var org in orgSource)
            {
                ...
                {
                    items.Add(new DirectoratReportDTO
                    {
            ...
            return workbook.Deliver("DirectoratReport-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _directoratReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("DirectoratReports.xlsx");
        }
