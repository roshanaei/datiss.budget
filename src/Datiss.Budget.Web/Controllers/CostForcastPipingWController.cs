...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class CostForcastPipingWController : Controller
    {

        public const string Name = "CostForcastPipingW";
        ...
        private readonly ILogger<CostForcastPipingWController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly ICostForcastPipingWService _costForcastPipingWService;
        private readonly IConstantService _constantService;
        private readonly IFinanceYearService _financeYearService;
        private readonly ISecurityTrimmingService _securityTrimmingService;

        public CostForcastPipingWController(
            ILogger<CostForcastPipingWController> logger,
            IWebHostEnvironment environment,
            ICostForcastPipingWService costForcastPipingWService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _costForcastPipingWService = costForcastPipingWService ?? throw new ArgumentNullException(nameof(costForcastPipingWService));
        ...
        public async Task<IActionResult> Create(CreateCostForcastPipingWViewModel model)
        {
            ...
            var data = model.Adapt<CreateCostForcastPipingWDTO>();

            var result = await _costForcastPipingWService.CreateAsync(data);
            ...
            return Json(result.Result.Adapt<CostForcastPipingWViewModel>());
        }


        [HttpPost("[action]")]
        [HasPermission(claimType: Name, actionType: PermissionActionType.Edit)]
        public async Task<IActionResult> Edit(UpdateCostForcastPipingWViewModel model)
        {
            ...
            var data = model.Adapt<UpdateCostForcastPipingWDTO>();
            var result = await _costForcastPipingWService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<CostForcastPipingWViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            var myfilter = TempData.Get<CostForcastPipingWFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<CostForcastPipingWFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _costForcastPipingWService.GetListAsync(filter);
            var model = result.Adapt<CostForcastPipingWIndexViewModel>();
        ...
        public async Task<IActionResult> Index(CostForcastPipingWIndexViewModel model)
        {

            var filter = model.Filter.Adapt<CostForcastPipingWFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _costForcastPipingWService.GetListAsync(filter);
            model = result.Adapt<CostForcastPipingWIndexViewModel>();
            model.Filter = filter.Adapt<CostForcastPipingWFilterViewModel>();
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _costForcastPipingWService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                await _costForcastPipingWService.HardDeleteAsync(id);
            }
            catch (DisbaledYearDataInputException)
            {
                return Json(new
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _costForcastPipingWService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.TargetYearId);
        ...
        {
            var result = await _costForcastPipingWService.GetExportItemsAsync(yearid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("CostForcastPipingW.xlsx");
        }
