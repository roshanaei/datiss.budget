...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class TotalBudgetWReportController : Controller
    {
        public const string Name = "TotalBudgetWReport";
        ...
        private readonly ILogger<TotalBudgetWReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly ITotalBudgetWReportService _totalBudgetWReportService;
        ...
        public TotalBudgetWReportController(
            ILogger<TotalBudgetWReportController> logger,
            IWebHostEnvironment environment,
            ITotalBudgetWReportService totalBudgetWReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _totalBudgetWReportService = totalBudgetWReportService ?? throw new ArgumentNullException(nameof(totalBudgetWReportService));
        ...
        public async Task<IActionResult> Edit(UpdateTotalBudgetWReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateTotalBudgetWReportDTO>();
            var result = await _totalBudgetWReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<TotalBudgetWReportViewModel>()
            );
        }
        ...
        {
            var filter = new TotalBudgetWReportFilterDTO();
            ...
            var myfilter = TempData.Get<TotalBudgetWReportFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<TotalBudgetWReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _totalBudgetWReportService.GetListAsync(filter);
            var model = result.Adapt<TotalBudgetWReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(TotalBudgetWReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<TotalBudgetWReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _totalBudgetWReportService.GetListAsync(filter);
            model = result.Adapt<TotalBudgetWReportIndexViewModel>();
            model.Filter = filter.Adapt<TotalBudgetWReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _totalBudgetWReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Calculation(CalculationInputViewModel model)
        {
            ...
            await _totalBudgetWReportService.CalculationAsync(
                        model.YearId,
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _totalBudgetWReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _totalBudgetWReportService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            foreach (var org in organizations)
            {
                ...
                {
                    items.Add(new TotalBudgetWReportDTO
                    {
            ...
            return workbook.Deliver("TotalBudgetWReport-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _totalBudgetWReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("TotalBudgetWReports.xlsx");
        }
