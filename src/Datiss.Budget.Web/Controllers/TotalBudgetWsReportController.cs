...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class TotalBudgetWsReportController : Controller
    {
        public const string Name = "TotalBudgetWsReport";
        ...
        private readonly ILogger<TotalBudgetWsReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly ITotalBudgetWsReportService _totalBudgetWsReportService;
        ...
        public TotalBudgetWsReportController(
            ILogger<TotalBudgetWsReportController> logger,
            IWebHostEnvironment environment,
            ITotalBudgetWsReportService totalBudgetWsReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _totalBudgetWsReportService = totalBudgetWsReportService ?? throw new ArgumentNullException(nameof(totalBudgetWsReportService));
        ...
        public async Task<IActionResult> Edit(UpdateTotalBudgetWsReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateTotalBudgetWsReportDTO>();
            var result = await _totalBudgetWsReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<TotalBudgetWsReportViewModel>()
            );
        }
        ...
        {
            var filter = new TotalBudgetWsReportFilterDTO();
            ...
            var myfilter = TempData.Get<TotalBudgetWsReportFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<TotalBudgetWsReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _totalBudgetWsReportService.GetListAsync(filter);
            var model = result.Adapt<TotalBudgetWsReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(TotalBudgetWsReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<TotalBudgetWsReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _totalBudgetWsReportService.GetListAsync(filter);
            model = result.Adapt<TotalBudgetWsReportIndexViewModel>();
            model.Filter = filter.Adapt<TotalBudgetWsReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _totalBudgetWsReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Calculation(CalculationInputViewModel model)
        {
            ...
            await _totalBudgetWsReportService.CalculationAsync(
                        model.YearId,
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _totalBudgetWsReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _totalBudgetWsReportService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            var sectionTypes = await _constantService.GetDataByKeyAsync(ConstantKeys.__GeneralWasteReportType);
            var unitTypes = await _constantService.GetDataByKeyAsync(ConstantKeys.__MeasurementType);



            var items = new List<TotalBudgetWsReportDTO>();

            foreach (var org in organizations)
            {
                foreach (var sec in sectionTypes)
                {
                    items.Add(new TotalBudgetWsReportDTO
                    {
            ...
            return workbook.Deliver("TotalBudgetWsReport-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _totalBudgetWsReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("TotalBudgetWsReports.xlsx");
        }
