...
namespace Datiss.Budget.Web.Controllers
{
    ...
    [Route("[controller]")]
    public class IncomeForcastController : Controller
    {

        public const string Name = "IncomeForcast";
        ...
        private readonly ILogger<IncomeForcastController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly IIncomeForcastService _incomeForcastService;
        private readonly IConstantService _constantService;
        private readonly IOrganizationService _organizationService;
        private readonly IFinanceYearService _financeYearService;
        private readonly ISecurityTrimmingService _securityTrimmingService;

        public IncomeForcastController(
            ILogger<IncomeForcastController> logger,
            IWebHostEnvironment environment,
            IIncomeForcastService incomeForcastService,
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> Create(CreateIncomeForcastViewModel model)
        {
            ...
            var data = model.Adapt<CreateIncomeForcastDTO>();
            ...
            return Json(result.Result.Adapt<IncomeForcastViewModel>());
        }


        [HttpPost("[action]")]
        public async Task<IActionResult> Edit(UpdateIncomeForcastViewModel model)
        {
            ...
            var data = model.Adapt<UpdateIncomeForcastDTO>();
            ...
            return Json(
                result.Result.Adapt<IncomeForcastViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            var myfilter = TempData.Get<IncomeForcastFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<IncomeForcastFilterDTO>();
            ...
            var model = result.Adapt<IncomeForcastIndexViewModel>();
        ...
        public async Task<IActionResult> Index(IncomeForcastIndexViewModel model, int page = 1)
        {
            model.Filter.PageNumber = 1;
            var filter = model.Filter.Adapt<IncomeForcastFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _incomeForcastService.GetListAsync(filter);
            model = result.Adapt<IncomeForcastIndexViewModel>();
            model.Filter = filter.Adapt<IncomeForcastFilterViewModel>();
        ...
        {
            var filePath = $"{_env.WebRootPath}\\Excel\\IncomeForcastImport.xlsx";
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            var items = new List<IncomeForcastDTO>();

            foreach (var org in organizations)
            {
                foreach (var utype in userTypes)
                {
                    items.Add(new IncomeForcastDTO
                    {
            ...
            return workbook.Deliver("IncomeForcast-Import-Template.xlsx");
        }
        ...
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            ...
            return workbook.Deliver("IncomeForcast.xlsx");
        }
        ...
        private string getCalcTitle(string key)
            => key switch
            {
                "IncomeForcast_Cal1" => SPTitles.IncomeForcast_Cal1,
                "IncomeForcast_Cal2" => SPTitles.IncomeForcast_Cal2,
