...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class CostCurrentProfitLossReportController : Controller
    {
        public const string Name = "CostCurrentProfitLossReport";
        ...
        private readonly ILogger<CostCurrentProfitLossReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly ICostCurrentProfitLossReportService _costCurrentProfitLossReportService;
        ...
        public CostCurrentProfitLossReportController(
            ILogger<CostCurrentProfitLossReportController> logger,
            IWebHostEnvironment environment,
            ICostCurrentProfitLossReportService costCurrentProfitLossReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _costCurrentProfitLossReportService = costCurrentProfitLossReportService ?? throw new ArgumentNullException(nameof(costCurrentProfitLossReportService));
        ...
        public async Task<IActionResult> Edit(UpdateCostCurrentProfitLossReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateCostCurrentProfitLossReportDTO>();
            var result = await _costCurrentProfitLossReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<CostCurrentProfitLossReportViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            var myfilter = TempData.Get<CostCurrentProfitLossReportFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<CostCurrentProfitLossReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _costCurrentProfitLossReportService.GetListAsync(filter);
            var model = result.Adapt<CostCurrentProfitLossReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(CostCurrentProfitLossReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<CostCurrentProfitLossReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _costCurrentProfitLossReportService.GetListAsync(filter);
            model = result.Adapt<CostCurrentProfitLossReportIndexViewModel>();
            model.Filter = filter.Adapt<CostCurrentProfitLossReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _costCurrentProfitLossReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Calculation(CalculationInputViewModel model)
        {
            ...
            var result = await _costCurrentProfitLossReportService.CalculationAsync(
                        model.YearId,
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _costCurrentProfitLossReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _costCurrentProfitLossReportService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            var sectionTypes = await _constantService.GetDataByKeyAsync(ConstantKeys.__CCProfitLossReportType);


            var items = new List<CostCurrentProfitLossReportDTO>();
            ...
            foreach (var org in organizations)
            {
                    foreach (var sec in sectionTypes)
                    {
                        string key = sec.ConstantKey.Substring(sec.ConstantKey.IndexOf('.') + 1);
                            items.Add(new CostCurrentProfitLossReportDTO
                            {
            ...
            return workbook.Deliver("CostCurrentProfitLossReport-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _costCurrentProfitLossReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("CostCurrentProfitLossReport.xlsx");
        }
    }
}
