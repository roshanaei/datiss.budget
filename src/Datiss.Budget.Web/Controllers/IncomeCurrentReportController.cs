...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class IncomeCurrentReportController : Controller
    {
        public const string Name = "IncomeCurrentReport";
        ...
        private readonly ILogger<IncomeCurrentReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly IIncomeCurrentReportService _incomeCurrentReportService;
        ...
        public IncomeCurrentReportController(
            ILogger<IncomeCurrentReportController> logger,
            IWebHostEnvironment environment,
            IIncomeCurrentReportService incomeCurrentReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _incomeCurrentReportService = incomeCurrentReportService ?? throw new ArgumentNullException(nameof(incomeCurrentReportService));
        ...
        public async Task<IActionResult> Edit(UpdateIncomeCurrentReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateIncomeCurrentReportDTO>();
            var result = await _incomeCurrentReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<IncomeCurrentReportViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            if (myfilter != null)
            {
                filter = myfilter.Adapt<IncomeCurrentReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _incomeCurrentReportService.GetListAsync(filter);
            var model = result.Adapt<IncomeCurrentReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(IncomeCurrentReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<IncomeCurrentReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _incomeCurrentReportService.GetListAsync(filter);
            model = result.Adapt<IncomeCurrentReportIndexViewModel>();
            model.Filter = filter.Adapt<IncomeCurrentReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _incomeCurrentReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _incomeCurrentReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        {
            var result = await _incomeCurrentReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("IncomeCurrentReports.xlsx");
        }
    }
}
