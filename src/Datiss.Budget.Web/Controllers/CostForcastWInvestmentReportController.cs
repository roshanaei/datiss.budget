...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class CostForcastWInvestmentReportController : Controller
    {
        public const string Name = "CostForcastWInvestmentReport";
        ...
        private readonly ILogger<CostForcastWInvestmentReportController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly ICostForcastWInvestmentReportService _costForcastWInvestmentReportService;
        private readonly IConstantService _constantService;
        private readonly IOrganizationService _organizationService;
        private readonly IFinanceYearService _financeYearService;
        private readonly ISecurityTrimmingService _securityTrimmingService;

        public CostForcastWInvestmentReportController(
            ILogger<CostForcastWInvestmentReportController> logger,
            IWebHostEnvironment environment,
            ICostForcastWInvestmentReportService costForcastWInvestmentReportService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _costForcastWInvestmentReportService = costForcastWInvestmentReportService ?? throw new ArgumentNullException(nameof(costForcastWInvestmentReportService));
        ...
        public async Task<IActionResult> Edit(UpdateCostForcastWInvestmentReportViewModel model)
        {
            ...
            var data = model.Adapt<UpdateCostForcastWInvestmentReportDTO>();
            var result = await _costForcastWInvestmentReportService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<CostForcastWInvestmentReportViewModel>()
            );
        }
        ...
        {
            var filter = new CostForcastWInvestmentReportFilterDTO();
            ...
            var myfilter = TempData.Get<CostForcastWInvestmentReportFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<CostForcastWInvestmentReportFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _costForcastWInvestmentReportService.GetListAsync(filter);
            var model = result.Adapt<CostForcastWInvestmentReportIndexViewModel>();
        ...
        public async Task<IActionResult> Index(CostForcastWInvestmentReportIndexViewModel model)
        {
            var filter = model.Filter.Adapt<CostForcastWInvestmentReportFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _costForcastWInvestmentReportService.GetListAsync(filter);
            model = result.Adapt<CostForcastWInvestmentReportIndexViewModel>();
            model.Filter = filter.Adapt<CostForcastWInvestmentReportFilterViewModel>();
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _costForcastWInvestmentReportService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _costForcastWInvestmentReportService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _costForcastWInvestmentReportService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            return workbook.Deliver("CostForcastWInvestmentReport-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _costForcastWInvestmentReportService.GetExportItemsAsync(yearid, orgid);
            if (result.Count() == 0)
                return RedirectToAction("Index");
            using var workbook = result.ExportExcel();
            return workbook.Deliver("CostForcastWInvestmentReports.xlsx");
        }
    }
}
