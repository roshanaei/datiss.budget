...
namespace Datiss.Budget.Web.Controllers
{
    [Authorize]
    [Route("[controller]")]
    public class IncomeCurrentCofficientController : Controller
    {
        public const string Name = "IncomeCurrentCofficient";
        ...
        private readonly ILogger<IncomeCurrentCofficientController> _logger;
        private readonly IWebHostEnvironment _env;
        private readonly IIncomeCurrentCofficientService _incomeCurrentCofficientService;
        ...
        public IncomeCurrentCofficientController(
            ILogger<IncomeCurrentCofficientController> logger,
            IWebHostEnvironment environment,
            IIncomeCurrentCofficientService incomeCurrentCofficientService,
            ...
            ISecurityTrimmingService securityTrimmingService)
        {
            ...
            _incomeCurrentCofficientService = incomeCurrentCofficientService ?? throw new ArgumentNullException(nameof(incomeCurrentCofficientService));
        ...
        public async Task<IActionResult> Create(CreateIncomeCurrentCofficientViewModel model)
        {
            ...
            var data = model.Adapt<CreateIncomeCurrentCofficientDTO>();

            var result = await _incomeCurrentCofficientService.CreateAsync(data);
            ...
            return Json(result.Result.Adapt<IncomeCurrentCofficientViewModel>());
        }


        [HttpPost("[action]")]
        [HasPermission(claimType: Name, PermissionActionType.Edit)]
        public async Task<IActionResult> Edit(UpdateIncomeCurrentCofficientViewModel model)
        {
            ...
            var data = model.Adapt<UpdateIncomeCurrentCofficientDTO>();
            var result = await _incomeCurrentCofficientService.UpdateAsync(data);
            ...
            return Json(
                result.Result.Adapt<IncomeCurrentCofficientViewModel>()
            );
        }
        ...
        public async Task<IActionResult> Index(int page = 1)
        {
            ...
            var myfilter = TempData.Get<IncomeCurrentCofficientFilterViewModel>(_indexFilterKey);
            if (myfilter != null)
            {
                filter = myfilter.Adapt<IncomeCurrentCofficientFilterDTO>();
                TempData.Put(_indexFilterKey, myfilter);
            }

            filter.PageNumber = page;

            var result = await _incomeCurrentCofficientService.GetListAsync(filter);
            var model = result.Adapt<IncomeCurrentCofficientIndexViewModel>();
        ...
        public async Task<IActionResult> Index(IncomeCurrentCofficientIndexViewModel model)
        {
            var filter = model.Filter.Adapt<IncomeCurrentCofficientFilterDTO>();

            TempData.Put(_indexFilterKey, filter);

            var result = await _incomeCurrentCofficientService.GetListAsync(filter);
            model = result.Adapt<IncomeCurrentCofficientIndexViewModel>();
            model.Filter = filter.Adapt<IncomeCurrentCofficientFilterViewModel>();
        ...
        public async Task<IActionResult> ImportExcel(ImportExcelViewModel model)
        {
            ...
            try
            {
                var result = await _incomeCurrentCofficientService.ImportExcelAsync(
                                                                    model.ExcelFile,
                                                                    model.YearId,
        ...
        public async Task<IActionResult> DeleteRecords(int yearId, int orgId)
        {
            try
            {
                var result = await _incomeCurrentCofficientService.HardDeleteAsync(yearId, orgId);
        ...
        public async Task<IActionResult> Delete(int id)
        {
            try
            {
                await _incomeCurrentCofficientService.HardDeleteAsync(id);
            }
            catch (DisbaledYearDataInputException)
            {
                return Json(new
        ...
        public async Task<IActionResult> Copy(CopyViewModel model)
        {
            ...
            try
            {
                await _incomeCurrentCofficientService.CopyAsync(
                                                    model.SourceYearId,
                                                    model.SourceOrgId,
                                                    model.TargetYearId);
        ...
        public async Task<IActionResult> GetExcelTemplate(int yearId, int? orgId)
        {
            ...
            foreach (var org in organizations)
            {
                userTypes.Where(ut => ut.ConstantKey == ConstantKeys.__House)
                    ...
                    .ForEach(ut => items.AddRange(houseUsageLayer
                                        .Select(hul => new IncomeCurrentCofficientDTO
                                        {
            ...
            return workbook.Deliver("IncomeCurrentCofficient-Import-Template.xlsx");
        }

        [HttpGet("[action]/{orgid}/{yearid}")]
        public async Task<IActionResult> ExportExcel(int orgid, int yearid)
        {
            var result = await _incomeCurrentCofficientService.GetExportItemsAsync(yearid, orgid);

            if (result.Count() == 0)
                return RedirectToAction("Index");

            using var workbook = result.ExportExcel();
            return workbook.Deliver("IncomeCurrentCofficient.xlsx");
